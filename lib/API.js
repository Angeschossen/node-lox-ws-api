...
var API = function (host, username, password, permissions, reconnect, security, keepalive) {
    this._host = host;
...
API.prototype.register_auth_object = function () {
    const auth_class = require('./Auth/' + (security === undefined ? 'Hash' : security) + '.js');
    if (this.security === "JWT") {
        this._auth = new (auth_class)(this._host, this._username, this._password, this._permissions, this.connection, this);
    } else {
        this._auth = new (auth_class)(this._host, this._username, this._password, this.connection, this);
    }
    ...
    https.get('https://' + this._host + '/jdev/cfg/api', (res) => {
        res.on('data', (chunk) => {
            if (res.statusCode === 200) {
                ...
                if (version[0] >= 10) {
                    that._auth_class = 'JWT';
                } else if (version[0] >= 9) {
                    that._auth_class = 'Token-Enc';
                }
                connection.connect();
            } else {
                that.reconnect();
