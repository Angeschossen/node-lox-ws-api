...
API.prototype.change_password = function (username, uuid, password, callback) {
    ...
    this._auth.send_get_key2(username, password, (key, pw_hash) => {
        that.send_command(`jdev/sps/updateuserpwdh/${uuid}/${pw_hash}`, false, {
            'control': new RegExp(`^j?dev\/sps\/updateuserpwdh\/${uuid}\/${pw_hash}$`),
            'callback': callback, 
            'onetime': true,
        });
    })
